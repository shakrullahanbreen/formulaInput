...
import CheckIcon from "@mui/material";
...
const MuiInput = () => {
  const {
    ...
    defaultValue: [initialFormulaInputs[1]],
    multiple: true,
    options: initialFormulaInputs,
    getOptionLabel: (option) => option.title,
  });
  const [userFormula, setUserFormula] = React.useState<FormulaInput[]>();
    ...
    <Root>
      <div {...getRootProps()}>
        ...
        <InputWrapper ref={setAnchorEl} className={focused ? "focused" : ""}>
          {value.map((option: FormulaInput, index: number) => (
      ...
      {groupedOptions.length > 0 ? (
        <Listbox {...getListboxProps()}>
          {(groupedOptions as typeof initialFormulaInputs).map(
            (option, index) => (
